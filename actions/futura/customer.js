...
async function main (params) {
  ...
  try {
    ...
    if(givexnumber != undefined) {
        givexupdatecustomerdata['request'] =  customerPayloadOfGivex;
        givexupdatecustomerdata['action'] =  "Update Customer in Givex";

        //extracting futura id from customerInfo(magento)
        var customerInfo = await getCustomer(params, params.data.value.entity_id);
        ...
        try{
            ...
            var updateCustomerGivexResult  = await call(params, 'dc_941', customerPayloadOfGivex);
    ...
    var duskpayload = await duskportalCustomerPayload(params, customerDuskPayload,customerID, data);
    var duskPortalCustomerUpdateResult;

    if(duskpayload != undefined){
        duskportalupdatecustomerdata['action'] = "Update customer in Dusk Portal";
